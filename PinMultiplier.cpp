...
// The output pin can also be used as a 9th bit value
// For glitch-free outputs, could use a second, larger resistor or another pin connected to the output latch RCLK
// For faster, simpler timing, could use separate pins for SER (data) and SRCLK (no resistor)
...
void setBits(unsigned char bits) {
  ...
  const unsigned char Equalize = 0x55;
	...
	for (int bit = 0; bit <= 8; bit++) {
    int thisBit = bits & 1;
    if (thisBit && !lastBit) Sleep(0);  // usleep(1) -- short delay to allow the filtered data pin to charge up
